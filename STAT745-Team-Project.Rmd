...
```{R}
# Read in the training data (adjust path as needed)
train_raw <- read.csv("train.csv")

# Use your cleaning function
train_cleaned <- clean_titanic(train_raw)

set.seed(42)  # for reproducibility
split_idx <- createDataPartition(train_cleaned$Survived, p = 0.8, list = FALSE)
train_data <- train_cleaned[split_idx, ]
val_data   <- train_cleaned[-split_idx, ]
...
# Train a GLM (logistic regression) model via caret
# Feel free to drop or add predictors based on exploration
glm_model <- train(
  Survived ~ Pclass + Sex + Age + LogFare + Embarked + Title + FamilySize + AgeGroup,
  data = train_data,
  method = "glm",
  family = binomial,
  metric = "Accuracy",
...
val_preds <- predict(glm_model, newdata = val_data)

# Confusion matrix
confusionMatrix(val_preds, val_data$Survived)
...
```{R}
train_raw <- read.csv("train.csv")
train_cleaned <- clean_titanic(train_raw, is_train = TRUE)

# Convert Survived to factor with names "No" and "Yes"
train_cleaned$Survived <- factor(
  train_cleaned$Survived,
  levels = c(0, 1),
  labels = c("No", "Yes")
)

# Fit a logistic regression on the entire training set
final_glm <- glm(
  Survived ~ Pclass + Sex + Age + LogFare + Embarked + Title + FamilySize + AgeGroup,
  data = train_cleaned,
  family = binomial
...
test_cleaned <- clean_titanic(test_raw, is_train = FALSE)

# Predict probabilities
test_probs <- predict(final_glm, newdata = test_cleaned, type = "response")
...
# Write to CSV
write_csv(submission, "submission_glm.csv")
